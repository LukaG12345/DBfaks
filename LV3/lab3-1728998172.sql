...
create table studenti (
   ...
   spol     number(10) not null
      constraint ck_spol check ( spol in ( 0,
                                           1 ) )
);
...
create table nastavnici (
   ...
   spol     number(10) not null,
   constraint ck_spol2 check ( spol in ( 0,
                                         1 ) )
);
...
create table upisi (
   ...
   datum      date null,
   ocjena     number(10)
      constraint ck_ocjena check ( ocjena in ( 1,
                                               2,
                                               3,
                                               4,
                                               5 ) )
...
alter table predmeti
   add constraint predmeti_fk1 foreign key ( idnastavnika )
      references nastavnici ( id );

alter table upisi
   add constraint upisi_fk1 foreign key ( idpredmeta )
      references predmeti ( id );

alter table upisi
   add constraint upisi_fk2 foreign key ( idstudenta )
