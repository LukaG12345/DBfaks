...
create table korisnici (
   ...
   constraint ck_spol3 check ( spol in ( 0,
                                         1 ) )
);
...
alter table adrese
   add constraint adrese_fk1 foreign key ( idkorisnika )
      references korisnici ( id );
alter table trivie
   add constraint trivie_fk1 foreign key ( idkorisnika )
      references korisnici ( id );

alter table korisnici add constraint uk_broj unique ( pozmob,
                                                      bromob );