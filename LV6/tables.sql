...
alter     table ucenici
add       constraint ucenici_fk4 foreign key (fk_idrazreda) references razredi (id);

alter     table predmeti
add       constraint predmeti_fk4 foreign key (fk_idrazreda) references razredi (id);

alter     table teme
add       constraint teme_fk2 foreign key (fk_idpredmeta) references predmeti (id);

alter     table odabiri
add       constraint odabiri_fk2 foreign key (fk_idtema) references teme (id);

alter     table odabiri
add       constraint odabiri_fk3 foreign key (fk_iducenika) references ucenici (id);